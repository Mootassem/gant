...
import DepenseRepository from '../database/repositories/depenseRepository';
...
export default class DepenseService {
  ...
  async destroyAll(ids) {
    ...
    try {
      for (const id of ids) {
        await DepenseRepository.destroy(id, {
          ...this.options,
          session,
        });
      }
  ...
  async findById(id) {
    return DepenseRepository.findById(id, this.options);
  }

  async findAllAutocomplete(search, limit) {
    return DepenseRepository.findAllAutocomplete(
      search,
      limit,
      this.options,
    );
  ...
  async findAndCountAll(args) {
    return DepenseRepository.findAndCountAll(
      args,
      this.options,
    );
  }
  ...
  async _isImportHashExistent(importHash) {
    const count = await DepenseRepository.count(
      {
        importHash,
      },
      this.options,
