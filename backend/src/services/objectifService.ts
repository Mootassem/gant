...
export default class ObjectifService {
  ...
  async destroyAll(ids) {
    ...
    try {
      for (const id of ids) {
        await ObjectifRepository.destroy(id, {
          ...this.options,
          session,
        });
      }
  ...
  async findById(id) {
    return ObjectifRepository.findById(id, this.options);
  }

  async findAllAutocomplete(search, limit) {
    return ObjectifRepository.findAllAutocomplete(
      search,
      limit,
      this.options,
    );
  ...
  async findAndCountAll(args) {
    return ObjectifRepository.findAndCountAll(
      args,
      this.options,
    );
  }
  ...
  async _isImportHashExistent(importHash) {
    const count = await ObjectifRepository.count(
      {
        importHash,
      },
      this.options,
