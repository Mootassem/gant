import GroupService from 'src/modules/group/groupService';
import Errors from 'src/modules/shared/error/errors';
import Message from 'src/view/shared/message';
import { getHistory } from 'src/modules/store';
import { i18n } from 'src/i18n';

const prefix = 'GROUP_FORM';

const groupFormActions = {
  INIT_STARTED: `${prefix}_INIT_STARTED`,
  INIT_SUCCESS: `${prefix}_INIT_SUCCESS`,
  INIT_ERROR: `${prefix}_INIT_ERROR`,

  CREATE_STARTED: `${prefix}_CREATE_STARTED`,
  CREATE_SUCCESS: `${prefix}_CREATE_SUCCESS`,
  CREATE_ERROR: `${prefix}_CREATE_ERROR`,

  UPDATE_STARTED: `${prefix}_UPDATE_STARTED`,
  UPDATE_SUCCESS: `${prefix}_UPDATE_SUCCESS`,
  UPDATE_ERROR: `${prefix}_UPDATE_ERROR`,

  doInit: (id) => async (dispatch) => {
    try {
      dispatch({
        type: groupFormActions.INIT_STARTED,
      });

      let record = {};

      const isEdit = Boolean(id);

      if (isEdit) {
        record = await GroupService.find(id);
      }

      dispatch({
        type: groupFormActions.INIT_SUCCESS,
        payload: record,
      });
    } catch (error) {
      Errors.handle(error);

      dispatch({
        type: groupFormActions.INIT_ERROR,
      });

      getHistory().push('/group');
    }
  },

  doCreate: (values) => async (dispatch) => {
    try {
      dispatch({
        type: groupFormActions.CREATE_STARTED,
      });

      await GroupService.create(values);

      dispatch({
        type: groupFormActions.CREATE_SUCCESS,
      });

      Message.success(
        i18n('entities.group.create.success'),
      );

      getHistory().push('/group');
    } catch (error) {
      Errors.handle(error);

      dispatch({
        type: groupFormActions.CREATE_ERROR,
      });
    }
  },

  doUpdate: (id, values) => async (dispatch, getState) => {
    try {
      dispatch({
        type: groupFormActions.UPDATE_STARTED,
      });

      await GroupService.update(id, values);

      dispatch({
        type: groupFormActions.UPDATE_SUCCESS,
      });

      Message.success(
        i18n('entities.group.update.success'),
      );

      getHistory().push('/group');
    } catch (error) {
      Errors.handle(error);

      dispatch({
        type: groupFormActions.UPDATE_ERROR,
      });
    }
  },
};

export default groupFormActions;
