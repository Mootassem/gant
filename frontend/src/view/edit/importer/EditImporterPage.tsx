import React from 'react';
import { i18n } from 'src/i18n';
import actions from 'src/modules/edit/importer/editImporterActions';
import fields from 'src/modules/edit/importer/editImporterFields';
import selectors from 'src/modules/edit/importer/editImporterSelectors';
import ContentWrapper from 'src/view/layout/styles/ContentWrapper';
import Breadcrumb from 'src/view/shared/Breadcrumb';
import importerHoc from 'src/view/shared/importer/Importer';
import PageTitle from 'src/view/shared/styles/PageTitle';

function EditImportPage() {
  const Importer = importerHoc(
    selectors,
    actions,
    fields,
    i18n('entities.edit.importer.hint'),
  );

  return (
    <>
      <Breadcrumb
        items={[
          [i18n('dashboard.menu'), '/'],
          [i18n('entities.edit.menu'), '/edit'],
          [i18n('entities.edit.importer.title')],
        ]}
      />

      <ContentWrapper>
        <PageTitle>
          {i18n('entities.edit.importer.title')}
        </PageTitle>

        <Importer />
      </ContentWrapper>
    </>
  );
}

export default EditImportPage;
