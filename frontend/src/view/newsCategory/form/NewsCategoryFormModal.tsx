...
import NewsCategoryForm from 'src/view/newsCategory/form/NewsCategoryForm';
import NewsCategoryService from 'src/modules/newsCategory/newsCategoryService';

function NewsCategoryFormModal(props) {
  ...
  const doSubmit = async (_, data) => {
    try {
      setSaveLoading(true);
      const { id } = await NewsCategoryService.create(data);
        ...
        <div className="modal-content">
          <div className="modal-header">
            <h5 className="modal-title">
              {i18n('entities.newsCategory.new.title')}
            </h5>
            <button
              type="button"
              className="close"
...
export default NewsCategoryFormModal;
