...
import FormuleForm from 'src/view/formule/form/FormuleForm';
import FormuleService from 'src/modules/formule/formuleService';

function FormuleFormModal(props) {
  ...
  const doSubmit = async (_, data) => {
    try {
      setSaveLoading(true);
      const { id } = await FormuleService.create(data);
        ...
        <div className="modal-content">
          <div className="modal-header">
            <h5 className="modal-title">
              {i18n('entities.formule.new.title')}
            </h5>
            <button
              type="button"
              className="close"
...
export default FormuleFormModal;
