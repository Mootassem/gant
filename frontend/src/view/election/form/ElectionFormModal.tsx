...
import ElectionForm from 'src/view/election/form/ElectionForm';
import ElectionService from 'src/modules/election/electionService';

function ElectionFormModal(props) {
  ...
  const doSubmit = async (_, data) => {
    try {
      setSaveLoading(true);
      const { id } = await ElectionService.create(data);
        ...
        <div className="modal-content">
          <div className="modal-header">
            <h5 className="modal-title">
              {i18n('entities.election.new.title')}
            </h5>
            <button
              type="button"
              className="close"
