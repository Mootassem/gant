...
import ShippingserviceForm from 'src/view/shippingservice/form/ShippingserviceForm';
import ShippingserviceService from 'src/modules/shippingservice/shippingserviceService';

function ShippingserviceFormModal(props) {
  ...
  const doSubmit = async (_, data) => {
    try {
      setSaveLoading(true);
      const { id } = await ShippingserviceService.create(data);
        ...
        <div className="modal-content">
          <div className="modal-header">
            <h5 className="modal-title">
              {i18n('entities.shippingservice.new.title')}
            </h5>
            <button
              type="button"
              className="close"
...
export default ShippingserviceFormModal;
